...
class GetSerialTvPopularBloc
  ...
  Future<void> _onRequested(GetSerialTVPopularRequested event,
      Emitter<GetSerialTVPopularState> emit) async {
    emit(GetSerialTVPopularLoading());

    final result = await _getSerialTV.execute();

    result.fold((failure) => emit(GetSerialTVPopularFailure(failure.message)),

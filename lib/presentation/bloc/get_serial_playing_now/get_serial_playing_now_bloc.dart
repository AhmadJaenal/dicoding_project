...
class GetSerialPlayingNowBloc
  ...
  Future<void> _onRequested(GetSerialPlayingNowRequested event,
      Emitter<GetSerialPlayingNowState> emit) async {
    emit(GetSerialPlayingNowLoading());

    final result = await _getSerialTVPlayingNow.execute();

    result.fold((failure) => emit(GetSerialPlayingNowFailure(failure.message)),

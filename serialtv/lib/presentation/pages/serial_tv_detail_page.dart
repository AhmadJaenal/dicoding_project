...
class _SerialTVDetailState extends State<SerialTVDetail> {
  ...
  void initState() {
    ...
    Future.microtask(() {
      context.read<GetDetailSerialBloc>().add(
        GetDetailSerialRequested(widget.id),
      );
      context.read<GetSerialRecommendationBloc>().add(
        GetSerialRecommendationRequested(widget.id),
      );
    });
  }
  ...
  Widget build(BuildContext context) {
    return BlocListener<WatchlistSerialTVBloc, WatchListState>(
      listener: (context, watchlistState) {
        ...
        if (watchlistState is WatchListFailure) {
          showDialog(
            context: context,
            builder: (_) => AlertDialog(content: Text(watchlistState.message)),
          );
        }
      },
      child: Scaffold(
              ...
              >(
                builder: (context, recommendationState) {
                  ...
                  if (recommendationState is GetSerialRecommendationFailure) {
                    return Center(child: Text('recommendationState.message'));
                  }
                    ...
                    Container(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(serialTV.name, style: heading5),
                            FilledButton(
                              onPressed: () async {
                                if (!isAddedWatchlist) {
                                  context.read<WatchlistSerialTVBloc>().add(
                                    WatchlistAddDataSerial(serialTV),
                                  );
                                } else {
                                  context.read<WatchlistSerialTVBloc>().add(
                                    WatchlistRemoveDataSerial(serialTV),
                                  );
                                }
                              },
                              child: Row(
